...
**1a. Отключить Vercel от этого репозитория.** К репозиторию, кроме Railway, подключён проект Vercel, который падает на каждом пуше — на 2026-08-09 накопилось 59 неуспешных деплойментов подряд (`gh api repos/xoxmach7/casa/deployments` показывает обоих ботов). Прод он не обслуживает, пользы не приносит, а красный статус на каждом коммите мешает отличать настоящую поломку от фонового шума.

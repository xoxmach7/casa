 ...
 *
 * Источников два, и каждый берётся ровно по тем правилам, по которым брокер
 * видит его в своём разделе — иначе подбор обещал бы объекты, которых в
 * каталоге для него не существует:
 *  - новостройки: Apartment со статусом AVAILABLE. Флаг project.isPublished
 *    сюда НЕ входит: он управляет видимостью на публичном сайте casa40, а не
 *    доступностью объекта брокеру (раздел «Новостройки» его не фильтрует).
 *  - вторичка: те же условия, что и на площадке вторички — активный объект с
 *    ДЕЙСТВУЮЩИМ договором собственника. Объект без договора для агента не
 *    существует (гейт бизнес-модели), поэтому попасть в подбор он не может.
 *
  ...
  const [newBuilds, secondary] = await Promise.all([
    ...
    prisma.crmProperty.findMany({
      where: {
        price: priceFilter,
        status: 'ACTIVE',
        // Гейт площадки вторички: без действующего договора собственника
        // объект для агента не существует.

  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.message || "Ошибка при создании объекта")
      }
    } catch (error) {
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-3xl font-bold tracking-tight">Новый объект</h2>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card className="md:col-span-2">
            <CardHeader>
              <CardTitle>Фотографии и видео</CardTitle>
            </CardHeader>
            <CardContent>
              <FileUpload
          ...
          <Card className="md:col-span-2">
            <CardHeader>
              <CardTitle>Видео и 3D-тур</CardTitle>
            </CardHeader>
            <CardContent>
              <FileUpload
                onUpload={(files) => {
                  const videoUrls = files.map(f => f.url)
                  // Store first video as main video
                  if (videoUrls.length > 0) {
                    setFormData(prev => ({
                      ...prev,
                      images: [...prev.images, ...videoUrls]
                    }))
                  }
                }}
                category="videos"
              ...
              />
              <p className="text-sm text-muted-foreground mt-2">
                Загрузите видео объекта (до 5 видео, макс. 50MB каждое)
              </p>
            </CardContent>
          </Card>
        </div>
